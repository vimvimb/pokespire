...
- **Offline-playable:** All assets are local; no runtime network requests. The game works fully without an internet connection after the first load.
- **Progressive Web App (PWA):** Installable, add-to-home-screen support, works offline
...
**Sprites** are sourced from PokemonDB and stored locally in `public/assets/sprites/` (`normal/` for front-facing, `back-normal/` for back-facing). Use `getSpriteUrl(pokemonId, variant)` from `src/ui/utils/sprites.ts`. To add new Pokemon sprites: see `public/assets/sprites/README.md` and run `npm run download-sprites`.
